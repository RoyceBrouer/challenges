...
const hippoExists = animalStrings.some((animal) => animal === "hippo");

// Hint: You can pass the starting index as second parameter.
const catStartingFromIndexFiveExists = animalStrings
  .slice(5)
  .some((animal) => animal === "cat");

// Hint: Besides the array method, check out the string method `startsWith()`.
const firstAnimalStartingWithLetterP = animalStrings.find(
  (animal) => animal.startsWith("p") / animal.startsWith("P")
);
...
//or >> prev + curr.length, 0)
// or animalStrings.map((animal) => animal.length).reduce((prev, curr) => prev + curr)
